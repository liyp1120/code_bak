...
#include <stdlib.h>
#include <stdarg.h>
#include<math.h> //liyp 2020.06.11
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *                                                                   *
 ...
 */
//liyp
int org_ort_get_guided_chunk(int niters, int chunksize, int *fiter, int *liter,
                         int *ignored, ort_gdopt_t *t)
{
	int   ch;
	ort_eecb_t         *me = __MYCB;
                         ...
                         int *ignored, ort_gdopt_t *t)
{
	...
	ort_eecb_t         *me = __MYCB;
	int static_ch; //liyp
	int myid = me->thread_num; //liyp
	static_ch = niters/2/(t->nth); //liyp
	...
	*fiter = *(t->data);
	...
	ee_unset_lock((ee_lock_t *) t->lock);
#endif
	//printf("thread id = %d, ch = %d\n",myid,ch);
	*liter = *fiter + ch;
	if(*liter > niters) *liter = niters; 
	return (ch != 0);
}

/* Return the sole chunk a thread gets assigned
 */
int org_ort_get_static_default_chunk(int niters, int *fiter, int *liter)
{
	ort_eecb_t *me = __MYCB;
	div_t      dchunksize;
	...
	//printf("myid = %d,fiter = %d,liter = %d\n",myid,*fiter,*liter);
	return (*fiter != *liter);
}
//liyp 2020.06.11  down loop ac
int ac_ort_get_static_default_chunk(int niters, int *fiter, int *liter)
{
        ort_eecb_t *me = __MYCB;
        div_t      dchunksize;
        int        chunksize, p = me->num_siblings, myid = me->thread_num;
        ...
        if (niters <= p)    /* less iterations than threads */
        {   
                *fiter = myid;
                *liter = (myid < niters) ? myid + 1 : myid;
                return (*fiter != *liter);
        }   
	double nit = (double)niters;
        double fi = nit*(sqrt(p-myid)-sqrt(p-myid-1))/sqrt(p);
        double start = nit - (nit * nit)/(2*p*fi) - fi/2;
	*fiter = start;
        *liter = *fiter + fi;
        if(myid == p-1) *liter = niters; 

	printf("myid = %d,fiter = %d,liter = %d,fi = %f\n",myid,*fiter,*liter,fi);
        return (*fiter != *liter);
}

//liyp 2020.06.11 up loop cp
int ort_get_static_default_chunk(int niters, int *fiter, int *liter)
{
	...
	double nit = (double)niters;
        double fi = nit*(sqrt(myid+1)-sqrt(myid))/sqrt(p);
        double start = (nit * nit)/(2*p*fi) - fi/2;
	*fiter = start;
        *liter = *fiter + fi;
        if(myid == p-1) *liter = niters; 
